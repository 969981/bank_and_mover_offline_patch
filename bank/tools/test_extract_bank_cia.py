...
class ExtractBankCiaTests(unittest.TestCase):
    ...
    def test_blz_literal_plus_backrefs(self):
        # Backward decode order: three literal 'A' bytes, then five 18-byte
        # backrefs at distance 3 => 93 'A' bytes total.
        stream = (b"\x00\xF0" * 5) + b"AAA" + bytes([0x1F])
        compressed_size = len(stream) + 8
        wanted_size = 93
        blob = stream + struct.pack(
            "<II", (8 << 24) | compressed_size, wanted_size - compressed_size
        )
