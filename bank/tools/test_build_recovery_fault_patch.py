...
class RecoveryFaultPatchTests(unittest.TestCase):
    ...
    def test_verified_fault_points_are_exact(self):
        self.assertEqual(FAULT_POINTS["post-stage"]["address"], 0x002B1E18)
        self.assertEqual(
            FAULT_POINTS["post-stage"]["stock"], bytes.fromhex("08 00 94 E5")
        )
        self.assertEqual(FAULT_POINTS["post-game-save"]["address"], 0x002B1F4C)
        self.assertEqual(
            FAULT_POINTS["post-game-save"]["stock"], bytes.fromhex("08 00 94 E5")
        )
        self.assertEqual(FAULT_POINTS["pre-complete"]["address"], 0x002B20A0)
        self.assertEqual(
    ...
    def test_branch_makefile_declares_variant_a(self):
        makefile = (Path(__file__).resolve().parents[1] / "src" / "Makefile").read_text(
            encoding="utf-8"
        )
        self.assertIn("FAULT_VARIANT := A", makefile)
